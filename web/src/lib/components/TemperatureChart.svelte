<script lang="ts">
	...
	const fetchTemperatures = async (device_name: string): Promise<TemperatureData[]> => {
		...
		return parsedResponse.map((entry: { created_at: string; temperature: number }) => ({
...
{#await temperatures_promise then device_temps}
	<div class="h-[300px] w-3/4 rounded-sm border p-4">
		<LineChart
			x="created_at"
			y="temperature"
			series={device_temps.map((dt, index) => ({
				key:
					dt.device.name === 'house'
						? 'Inomhus'
						: dt.device.name === 'birdhouse'
							? 'Utomhus'
							: dt.device.name,
