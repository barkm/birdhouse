    ...
    <div class="space-y-4">
        <div class="grid grid-cols-2 gap-4">
            ...
            {:then outsideSensorData} 
                <SensorCard title={"Utomhus"} temperature={outsideSensorData.temperature} humidity={outsideSensorData.humidity} />
            {/await}
            {#await inside_sensor_data_promise}
               <SensorLoader /> 
            {:then insideSensorData} 
                <SensorCard title={"Inomhus"} temperature={insideSensorData.temperature} humidity={insideSensorData.humidity} />
            {/await}
        </div>
