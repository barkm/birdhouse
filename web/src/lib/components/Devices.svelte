<script lang="ts">
    ...
    const statuses_promise = $derived(active_devices_promise.then(active => {
        return Promise.all(active.map(device => 
    ...
    const devices_promise = $derived(Promise.all([recorded_devices_promise, statuses_promise]).then(([recorded, active]) => {
