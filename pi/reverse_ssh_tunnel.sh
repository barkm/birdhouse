...
if [[ "$1" == "install" ]]; then
  ...
  sudo $UV run service install \
      --name $SERVICE_NAME \
      --command "$AUTO_SSH_PATH -M 0 -N -o "StrictHostKeyChecking=accept-new" -o "ServerAliveInterval=60" -o "ServerAliveCountMax=3" -R ${SERVER_PORT}:localhost:8000 -R ${SSH_PORT}:localhost:22 -L 5000:localhost:5000 ${REMOTE_HOST}" \
