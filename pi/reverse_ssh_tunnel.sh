...
if [[ "$1" == "install" ]]; then
  ...
  sudo $UV run service install \
      --name $SERVICE_NAME \
      --command "$(which autossh) -M 0 -N -o "ServerAliveInterval=60" -o "ServerAliveCountMax=3" -R ${SERVER_PORT}:localhost:8000 -R ${SSH_PORT}:localhost:22 -L 5000:localhost:5000 ${REMOTE_HOST}" \
